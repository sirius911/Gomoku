    ...
    if initial_move:
        ...
        for x in range(center - radius, center + radius + 1):
            for y in range(center - radius, center + radius + 1):
                if (x, y) not in board and \
                    not check_double_three(board, x, y, current_player): #case vide et pas de Double Three
                    possible_moves.append((x, y))
    else:
        # Votre logique habituelle pour générer des mouvements
        for x in range(size):  # Assumant que board_size est défini ailleurs
            for y in range(size):
                if (x, y) not in board and \
                    not check_double_three(board, x, y, current_player): #case vide et pas de Double Three
                    possible_moves.append((x, y))
